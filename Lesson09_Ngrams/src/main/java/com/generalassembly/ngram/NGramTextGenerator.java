...
public class NGramTextGenerator {
     ...
     */
    // the ngrams are the keys to the hash maps; the value is the word that follows it
    //e.g.
    ...
    public String randomText() {
        ...
        for (int i = 0; i < OUTPUT_WORD_COUNT; i++) {
            ...
            // * If words is an ngram that exists in the map, get its associated ArrayList<String> and use random.nextInt
            ...
            ArrayList<String> trailingWords;

            if (!ngrams.containsKey(words)){
                words = this.randomNGram();
            }
            else {
                trailingWords = ngrams.get(words);
                int randWord = random.nextInt(trailingWords.size());
                String nextWord = trailingWords.get(randWord);
                stringWriter.append(nextWord + " ");
            }
            words = this.randomNGram();
        }
